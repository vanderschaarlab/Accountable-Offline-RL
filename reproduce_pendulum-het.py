...
class WorldModel_Hybrid(nn.Module):
    def __init__(
        ...
        self.context = Context(
            hidden_sizes=[self.hidden_recurrent],
...
def eval_policy(policy=None, hist_len=None, env_name=None):
    ...
    for step_i in range(ENV_MAX_STEP):
        a = (
            policy.forward_single(
                torch.as_tensor(state_eval).float().cuda().unsqueeze(0),
                torch.as_tensor(previous_info).float().cuda().unsqueeze(0),
            )
            .cpu()
            .detach()
            .numpy()[0]
...
def accountable_batched_controller(
    ...
    if use_latent:
        ...
        for i in range(0, len(state), bs_):
            buffer_latent_rep.append(
                world_model.get_representation(
                    state[i : i + bs_].cuda(), action[i : i + bs_].cuda(), preinfo[i : i + bs_].cuda()
                )
                .cpu()
                .detach()
            )
...
class TD3(object):
    ...
    def train(self, buffer_sa, buffer_r, preinfo_set, bs=100, update_policy=True):
        ...
        with torch.no_grad():
            ...
            next_action = (self.actor_target(next_state, pre_info_next) + noise).clamp(
    ...
    for n in range(n_refine):
        ...
        for t in range(horizon):
            ...
            for j in range(num_rollouts):
                prev_inf[j].append(
                    np.hstack((state[j].cpu().detach().numpy(), action[j].cpu().detach().numpy(), reward[j]))
                )
        ...
        mu = (
            np.sum(exp_reward[np.newaxis, :, np.newaxis].repeat(horizon, 0).repeat(action_dim, -1) * action_list, 1)
            / exp_reward_sum
...
if __name__ == "__main__":
    ...
    for i in range(LEN):
        preinfo_set.append(
            np.concatenate((padded_buffer_sa[i : i + args.hist_len], padded_buffer_r[i : i + args.hist_len]), 1)
        )

    preinfo_set = np.array(preinfo_set)

    world_model = WorldModel_Hybrid(
    ...
    for epoch in range(4000):
        ...
        if epoch % 100 == 0:
            print("epoch", epoch, "loss", loss.item())
    esc_results_latent_100_mb = accountable_batched_controller(
        buffer_sa,
        preinfo_set,
        buffer_r_cumavg,
    ...
    esc_results_latent_100 = accountable_batched_controller(
        buffer_sa,
        preinfo_set,
        buffer_r_cumavg,
        n_rollout=args.n_rollout,
    ...
    knn_latent_100 = accountable_batched_controller(
        buffer_sa,
        preinfo_set,
        buffer_r_cumavg,
        n_rollout=args.n_rollout,
    ...
    knn_result_100 = accountable_batched_controller(
        buffer_sa,
        preinfo_set,
        buffer_r_cumavg,
        n_rollout=args.n_rollout,
    ...
    knn_result_1 = accountable_batched_controller(
        buffer_sa,
        preinfo_set,
        buffer_r_cumavg,
        n_rollout=args.n_rollout,
    ...
    results = {
        "esc_results_latent_100": esc_results_latent_100,
    ...
    # Model-Based + MPC
    reward_model = WorldModel_Hybrid(
        state_dim=ENV_STATE_DIM,
        action_dim=ENV_ACTION_DIM,
        hidden_dim=128,
        hidden_recurrent=128,
    ...
    world_model = WorldModel_Dynamics(
        state_dim=ENV_STATE_DIM,
        action_dim=ENV_ACTION_DIM,
        hidden_dim=128,
        hidden_recurrent=128,
    ...
    for j in range(N_ROLL):
        for i in range(args.hist_len):
            previous_info[j].append(np.hstack((np.zeros_like(init_state), np.zeros(ENV_ACTION_DIM), np.zeros(1))))
    ...
    for t in tqdm(range(ENV_MAX_STEP)):
        ...
        estimated_next_state = (
            state
            + world_model(
                torch.as_tensor(state).cuda().unsqueeze(0),
                torch.as_tensor(action).cuda().unsqueeze(0),
                torch.as_tensor(np.asarray(previous_info)).float().cuda()[0].unsqueeze(0),
            )
            .cpu()
            .detach()
            .numpy()
        ...
        for j in range(N_ROLL):
            previous_info[j].append(np.hstack((state, action, reward)))
        state = next_state
        total_reward += reward
        if done1 or done2:
            break
