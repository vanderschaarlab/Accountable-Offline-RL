...
def LP_solver_minimalhull(
    ...
    for i in range(n_test):
        ...
        min_hull, indices = find_min_hull_heuristic(
            train_latent_reps.cpu().detach().numpy(), test_latent_reps[i].cpu().detach().numpy(), start_k
        )
        if min_hull is None:
...
train_x = np.concatenate([static_train[:, np.newaxis, :].repeat(MAX_LEN, 1), observations_train], axis=2).reshape(
    -1, combined_feature_shape
)
train_y = actions_train.reshape(-1, 1)
test_x = np.concatenate([static_test[:, np.newaxis, :].repeat(MAX_LEN, 1), observations_test], axis=2).reshape(
    -1, combined_feature_shape
)
...
plt.scatter(
    data_tsne[: int(N_TRAIN * sampling_rate), 0],
    data_tsne[: int(N_TRAIN * sampling_rate), 1],
    label="Training Data",
    alpha=0.5,
)
plt.scatter(
