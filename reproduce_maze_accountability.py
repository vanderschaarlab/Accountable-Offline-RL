...
def task1234_policy(state, eps=0.15, decay=0.5, task_config=None):
    target_position = task_config["rewarding_loc"]
    direc = (np.array(target_position) - np.array(state)) / np.linalg.norm(
        (np.array(target_position) - np.array(state))
    )
...
for qt in [0.1, 0.3, 0.5]:
    ...
    for repeat in range(100):
        rwd, traj, a_idx = accountable_batched_controller(
            buffer_sa,
            buffer_r_cumavg,
            n_rollout=5000,
            n_keep=100,
...
for i in range(len(sorted_state_list)):
    ...
    if i < len(sorted_state_list) - 1:
        if np.linalg.norm(sorted_state_list[i + 1] - np.array([0.0, 0.0])) == 0:
            splitted_trajs.append(subtraj)
            subtraj = []
        if np.linalg.norm(sorted_state_list[i + 1] - np.array([8.0, 8.0])) == 0:
            splitted_trajs.append(subtraj)
            subtraj = []
        if np.linalg.norm(sorted_state_list[i + 1] - np.array([8.0, 16.0])) == 0:
            splitted_trajs.append(subtraj)
            subtraj = []
...
for traj_j in range(2):
    ...
    for step_i in range(len(traj)):
        ...
        n_policy2 = (
            (len(sorted_state_list) / 4 < ref_list_now) & (ref_list_now < len(sorted_state_list) / 4 * 2)
        ).sum()
        n_policy3 = (
            (len(sorted_state_list) / 4 * 2 < ref_list_now) & (ref_list_now < len(sorted_state_list) / 4 * 3)
        ).sum()
        n_policy4 = (
            (len(sorted_state_list) / 4 * 3 < ref_list_now) & (ref_list_now < len(sorted_state_list) / 4 * 4)
        ).sum()
